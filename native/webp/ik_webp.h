...
int ik_webp_version_str(uint8_t* out, int cap);

// Header probing. Returns 0 on success; sets out_w/out_h.
int ik_webp_get_info(const uint8_t* data, int len,
                     int* out_w, int* out_h,
                     uint8_t* err, int err_cap);

// Decode into caller-provided buffer with optional pitch (0=tight).
int ik_webp_decode_to_pixels(const uint8_t* data, int len,
...
int ik_webp_encode_from_pixels_ex(const uint8_t* pixels,
...
int ik_webp_transform_simple(const uint8_t* data, int len,
                             int op, int options,
